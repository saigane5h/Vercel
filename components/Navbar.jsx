        ...
        <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-1">
              {navLinks.map(link => (
                <Link key={link.href} href={link.href}
                  className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${pathname === link.href ? 'text-gold-400 bg-gold-400/10' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
                  {link.label}
                </Link>
              ))}
            </div>
          ...
          <div className="flex items-center gap-3">
            {user ? (
              ...
              <Link href="/login" className="px-4 py-1.5 rounded-lg bg-gold-400 text-navy-900 text
