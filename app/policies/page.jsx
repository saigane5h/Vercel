    ...
    <ProtectedRoute>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="flex items-start gap-3 p-4 rounded-xl bg-red-400/8 border border-red-400/20 mb-10">
            ...
            <div>
              ...
              <p className="text-xs text-white/40 leading-relaxed">All BFSI professionals are required to review the latest IRDAI circulars. New policy videos are added regularly. Ensure your team stays compliant.</p>
            </div>
          </div>
              ...
              <div key={pv.id} className="card-glow rounded-xl bg-navy-800/60 overflow-hidden">
                <div className="flex flex-col sm:flex-row items-start">
                  <button onClick={() => setActiveVideo(pv)}
                    ...
                    <div className="absolute bottom-2 right-2 bg-black/60 rounded px-2 py-0.5 text-xs text-white/70">{pv.duration}</div>
                  </button>
                  <div className="flex-1 p-5">
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-2">
                      <span className={`badge ${pv.tagColor}`}>{pv.tag}</span>
                      <div className="flex items-center gap-1.5 text-xs text-white/30">
                        <Calendar size={11} /><span>{pv.date}</span>
                      </div>
                    </div>
                    <h3 className="font-display text-lg font-semibold text-white mb-2 leading-snug">{pv.title}</h3>
                    <p className="text-sm text-white/45 leading-relaxed mb-4">{pv.summary}</p>
                    <div className="flex items-center gap-4">
                      <button onClick={() => setActiveVideo(pv)} className="flex items-center gap-1.5 text-gold-400 text-sm font-medium hover:text-gold-500 transition-colors">
                        <Play size={13} /> Watch Now
                      </button>
                      <span className="flex items-center gap-1 text-xs text-white/30"><Clock size={11} /> {pv.duration}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {activeVideo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setActiveVideo(null)}>
          <div className="relative w-full max-w-3xl bg-navy-800 rounded-2xl border border-white/10 overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between p-4 border-b border-white/5">
              <div className="flex-1 pr-4">
                ...
                <h3 className="font-display text-base font-semibold text-white leading-snug">{activeVideo.title}</h3>
              </div>
              <button onClick={() => setActiveVideo(null)} className="text-white/30 hover:text-white transition-colors flex-shrink-0">
                <X size={20} />
              </button>
            </div>
