...
// ─────────────────────────────────────────────
// VideoPlayer — paste any iframe embed code into
// the `embedCode` field in lib/data.js.
// If embedCode is empty, a styled placeholder is shown.
// ─────────────────────────────────────────────
function VideoPlayer({ video }) {
  ...
  if (video.embedCode) {
    // Renders any iframe embed code (YouTube, Vimeo, Wistia, Loom, custom CDN, etc.)
    return (
      <div
        className="rounded-2xl overflow-hidden border border-white/8 bg-black video-wrapper"
        dangerouslySetInnerHTML={{ __html: video.embedCode }}
      />
    )
  }

  // Placeholder shown until embedCode is added
  return (
      ...
      <ProtectedRoute>
        <div className="min-h-screen flex items-center justify-center text-center px-4">
          <div>
            ...
            <p className="text-white/40 text-sm mb-6">This course may have moved or doesn't exist.</p>
                  ...
                  <div className="divide-y divide-white/5 max-h-[60vh] overflow-y-auto">
                    {courseVideos.length > 0 ? courseVideos.map((video, idx) => (
                      <button
                        ...
                        className={`w-full text-left px-5 py-4 flex items-start gap-3 hover:bg-white/3 transition-colors ${
                          activeVideo?.id === video.id ? 'bg-gold-400/8 border-l-2 border-gold-400' : ''
                        }`}
                      >
                        <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold mt-0.5 ${
                          activeVideo?.id === video.id
                            ? 'bg-gold-400 text-navy-900'
                            : 'bg-navy-700 text-white/40'
                        }`}>
                          {idx + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm leading-snug mb-1 ${
                            activeVideo?.id === video.id ? 'text-gold-400' : 'text-white/70'
                          }`}>
                            {video.title}
                          </p>
